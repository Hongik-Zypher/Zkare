...
const MEDICAL_DATA_VERIFIER_ADDRESS = process.env.REACT_APP_MEDICAL_DATA_VERIFIER_ADDRESS || 
...
export const getMyPendingRequests = async () => {
  ...
  try {
    ...
    try {
      pendingCount = await medicalDataVerifierContract.getPendingRequestCount(myAddress);
    } catch (countError) {
    ...
    let i = 0;
    while (validRequestCount < Number(pendingCount) && i < 100) { // 최대 100개까지만 검사
      try {
        const details = await medicalDataVerifierContract.getRequestDetails(myAddress, i);
...
export const respondToVerification = async (requestId, approved) => {
  ...
  try {
    const tx = await medicalDataVerifierContract.respondToVerification(requestId, approved);
    await tx.wait();
...
export const verifyBloodType = async (patientAddress, guessedBloodType) => {
  try {
    // 1. 검증 요청
    const requestResult = await requestVerification(patientAddress, "bloodType", guessedBloodType);
 ...
 */
export const verifyMedicalData = async (dataType, patientAddress, proof, publicInput) => {
  if (!medicalDataVerifierContract) await initVerificationService();
...
export const registerDoctor = async (doctorAddress) => {
  ...
  try {
    ...
    const tx = await zkareContract.addDoctor(doctorAddress || await getCurrentAccount());
    await tx.wait();
    
    console.log(`의사 등록 완료: ${doctorAddress || await getCurrentAccount()}`);
    ...
    try {
      if (account === await getCurrentAccount()) {
        ...
        if (typeof zkareContract.registerPatient === 'function') {
          ...
          return {
            success: false,
            message: '환자 등록 함수가 컨트랙트에 없습니다. 컨트랙트 버전을 확인해주세요.'
          };
        }
        ...
        return {
          success: false,
          message: '다른 주소를 환자로 등록하려면 의료 기록 등록 등을 통해 간접적으로 등록해야 합니다.'
        };
      }
    } catch (registerError) {
  ...
  try {
    ...
    try {
      ...
      if (typeof zkareContract.getAllPatients === 'function') {
          ...
          patients = await Promise.all(
            patientAddresses.map(async (address) => {
              ...
              try {
                ...
                // 환자의 혈액형 정보 조회 
                try {
                  const bloodTypeResult = await medicalDataVerifierContract.getPatientData(address, "bloodType");
        ...
        console.log('getAllPatients 함수를 찾을 수 없어 환자 목록을 가져올 수 없습니다.');
      }
    } catch (contractError) {
